...
void setupPrims() {
	...
	LilSpheres::setupParticles(LilSpheres::maxParticles); //POSITION
	//
	...
	for(int i = 0; i < LilSpheres::maxParticles; ++i) {
		partVerts[i * 3 + 0] = (((float)rand() / RAND_MAX) * 10.f - 5.f); //x
		partVerts[i * 3 + 1] = (((float)rand() / RAND_MAX) * 10.f); //y
		partVerts[i * 3 + 2] = (((float)rand() / RAND_MAX) * 10.f - 5.f); //z
 	}
